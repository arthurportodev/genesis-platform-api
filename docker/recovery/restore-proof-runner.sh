...
"$docker_bin" run --rm --name "genesis-recovery-restore-$run_id" --label "$RECOVERY_LABEL_KEY=$run_id" --platform "$RECOVERY_PLATFORM" --network "$network" --group-add "$RECOVERY_CONTAINER_SECRET_GID" --read-only --cap-drop ALL --security-opt no-new-privileges:true --tmpfs /run/genesis:rw,noexec,nosuid,nodev,size=1m,mode=0700 --mount "type=bind,src=$dump_file,dst=/recovery/backup.dump,readonly" --mount "type=bind,src=$RECOVERY_RESTORE_SECRETS_DIR/restore-bootstrap-pgpass,dst=/run/recovery-secrets/restore-bootstrap-pgpass,readonly" --entrypoint /bin/sh "$RECOVERY_POSTGRES_IMAGE" -c 'set -eu; umask 077; cp /run/recovery-secrets/restore-bootstrap-pgpass /run/genesis/pgpass; chmod 0600 /run/genesis/pgpass; export PGPASSFILE=/run/genesis/pgpass; exec "$@"' sh pg_restore --no-password --exit-on-error --no-owner --role genesis_migration --host "$postgres" --port 5432 --username genesis_bootstrap --dbname genesis_platform /recovery/backup.dump
rm -f -- "$dump_file"

runtime_select_denylist="'migrations','lead_ingest_idempotency','lead_command_idempotency','lead_follow_up_idempotency'"
verification_sql="SET ROLE genesis_migration; GRANT CONNECT ON DATABASE genesis_platform TO genesis_runtime; RESET ROLE; SELECT CASE WHEN has_database_privilege('genesis_runtime',current_database(),'CONNECT') AND to_regclass('public.migrations') IS NOT NULL AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_type='BASE TABLE') AND pg_get_userbyid((SELECT datdba FROM pg_database WHERE datname=current_database()))='genesis_migration' AND pg_get_userbyid((SELECT nspowner FROM pg_namespace WHERE nspname='public'))='genesis_migration' AND NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind IN ('r','p','S') AND pg_get_userbyid(c.relowner)<>'genesis_migration') AND (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind IN ('r','p') AND c.relname IN ($runtime_select_denylist))=4 AND NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind IN ('r','p') AND c.relname NOT IN ($runtime_select_denylist) AND NOT has_table_privilege('genesis_runtime',c.oid,'SELECT')) AND NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind IN ('r','p') AND c.relname IN ($runtime_select_denylist) AND has_table_privilege('genesis_runtime',c.oid,'SELECT')) AND NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relrowsecurity AND NOT EXISTS (SELECT 1 FROM pg_policy p WHERE p.polrelid=c.oid)) THEN 'verified' ELSE 'invalid' END"
schema_result=$("$docker_bin" run --rm --name "genesis-recovery-verify-$run_id" --label "$RECOVERY_LABEL_KEY=$run_id" --platform "$RECOVERY_PLATFORM" --network "$network" --group-add "$RECOVERY_CONTAINER_SECRET_GID" --read-only --cap-drop ALL --security-opt no-new-privileges:true --tmpfs /run/genesis:rw,noexec,nosuid,nodev,size=1m,mode=0700 --mount "type=bind,src=$RECOVERY_RESTORE_SECRETS_DIR/restore-bootstrap-pgpass,dst=/run/recovery-secrets/restore-bootstrap-pgpass,readonly" --entrypoint /bin/sh "$RECOVERY_POSTGRES_IMAGE" -c 'set -eu; umask 077; cp /run/recovery-secrets/restore-bootstrap-pgpass /run/genesis/pgpass; chmod 0600 /run/genesis/pgpass; export PGPASSFILE=/run/genesis/pgpass; exec "$@"' sh psql --quiet --no-password --tuples-only --no-align --host "$postgres" --username genesis_bootstrap --dbname genesis_platform --set ON_ERROR_STOP=1 --command "$verification_sql")
...
for _ in $(seq 1 60); do
  ...
  if [ "$("$docker_bin" inspect --format '{{.State.Running}}' "$api" 2>/dev/null || true)" != true ]; then
    "$docker_bin" logs --tail 100 "$api" >&2 || true
    fail 'ephemeral API exited before readiness'
  fi
  sleep 2
done
