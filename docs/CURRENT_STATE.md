...
- **Atualização documentada:** 2026-08-12T07:56:13.6858221Z
- **Fase:** 0.8-MVP — Primeira produção mínima viável
- **Último trabalho concluído:** 0.8-MVP-06B-WINDOW-2A-ATTEMPT-1 — Window 2A revertida — exposição pública restaurada ao estado fechado
- **Trabalho vigente:** in-progress — A tentativa 1 da Window 2A comprovou DNS, TCP/80, Traefik public-http, HTTP-01 e ACME staging, mas foi encerrada em REVERT por violação do contrato de coleta de evidências; a superfície pública foi restaurada ao estado fechado e a 0.8-MVP-06B continua em andamento.
- **Próxima tarefa:** 0.8-MVP-06B-WINDOW-2A-EVIDENCE-CLOSEOUT — Validar protocolo seguro de evidências antes da repetição da Window 2A
- **Web integrado:** fa4193fc28751d64923be824d293367499d4fba0
- **Proveniência da API:** containing-commit

## Estado operacional

A tentativa 1 da Window 2A terminou em REVERT por falha no procedimento de coleta de evidências, não por falha comprovada de DNS, TCP/80, Traefik, HTTP-01 ou ACME staging. DNS da API voltou a NXDOMAIN/NODATA, TCP/80 e TCP/443 públicos estão fechados, Traefik está novamente internal em loopback, API e PostgreSQL permanecem privados e saudáveis, o estado ACME staging foi preservado sem leitura de conteúdo e ACME production permanece ausente.

- **OPS-PRIVATE-BASELINE** [documented/present] — API e PostgreSQL são documentados como instalados em uma baseline privada.
- **OPS-PRIVATE-BASELINE-LIVE** [observed/present] — API e PostgreSQL permaneceram running e healthy após o rollback da Window 2A, com IDs preservados, restart count zero e sem bindings de porta no host; a rede do banco permanece privada.
- **OPS-PUBLIC-SSH-ONLY** [observed/present] — Após o rollback da Window 2A, a superfície pública voltou a ficar restrita a TCP/22; TCP/80, 443, 3000, 5432 e 8080 permaneceram fechadas na verificação final.
...
- **OPS-TRAEFIK-LIVE** [observed/present] — Após o rollback da Window 2A, o Traefik está running em modo internal, com restart count zero e bindings exclusivos em 127.0.0.1:18080 e 127.0.0.1:18443; a matriz health-only passou e nenhuma rota funcional ficou pública.
- **OPS-ACME-STATE** [observed/present] — A tentativa 1 comprovou ACME staging tecnicamente e preservou acme-staging.json com 16.293 bytes, root:root e modo 0600, sem ler ou hashear conteúdo; acme.json production permanece root:root, modo 0600 e zero bytes, sem solicitação production.
- **OPS-PUBLIC-CERTIFICATE** [documented/absent] — Nenhum certificado ACME production foi solicitado ou emitido; a tentativa 1 retornou somente certificado de staging, sem abrir TCP/443 público.
- **OPS-PUBLIC-HTTPS** [observed/absent] — Nenhum endpoint HTTPS público da API foi habilitado; TCP/443 público permaneceu fechado.
- **OPS-PUBLIC-HTTP-BINDINGS** [observed/absent] — Após o rollback, não há listeners ou bindings públicos TCP/80 e TCP/443; somente os listeners privados de loopback do Traefik foram restaurados.
- **OPS-06B-DEPLOYMENT** [observed/partial] — A Window 1 permanece em KEEP; a tentativa 1 da Window 2A foi revertida integralmente após comprovar ACME staging. A 06B continua parcial, sem publicação, deploy público, exposição funcional ou dados reais.
- **OPS-06B-WINDOW2A-ATTEMPT1** [observed/partial] — A tentativa 1 da Window 2A comprovou DNS convergente, TCP/80 alcançável, Traefik public-http, HTTP-01 válido e certificado ACME staging; um coletor registrou um valor sensível efêmero e acionou REVERT antes da observação de 30 minutos.
- **OPS-06B-WINDOW2A-REVERT** [observed/present] — O rollback aprovado restaurou DNS ausente, TCP/80 e TCP/443 públicos fechados e Traefik internal/loopback; a causa foi violação do contrato de coleta de evidências, sem falha estrutural comprovada em DNS, TCP/80, Traefik ou HTTP-01.
...
- **OPS-DNS-API** [observed/absent] — O registro A da API criado na tentativa 1 da Window 2A foi removido no rollback; api.agenciagenesismkt.com.br voltou a NXDOMAIN/NODATA nas verificações finais.
...
- **BLOCK-PUBLIC-ORIGIN** — A tentativa 1 da Window 2A foi revertida por violação do contrato de coleta de evidências; a superfície pública está fechada e uma repetição exige protocolo seguro validado, novo Gate humano e autorização operacional explícita.
...
- **HD-AUTHORIZE-WINDOW2A-RETRY** — A repetição da Window 2A será autorizada após aprovação humana do protocolo seguro de evidências?
...
- **OR-WINDOW2-NOT-AUTHORIZED** — A próxima tarefa limita-se a validar o protocolo seguro de evidências; repetir a Window 2A ou alterar DNS, TCP/80, ACME, certificado, TCP/443, exposição pública ou observação pública não está autorizado por esta reconciliação.
