  ...
  try {
    ...
    psql(
      ...
      [
        ...
        'CREATE TABLE lead_ingest_idempotency (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY)',
        'CREATE TABLE lead_command_idempotency (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY)',
        'CREATE TABLE lead_follow_up_idempotency (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY)',
        ...
        'GRANT SELECT ON tenant_records TO genesis_runtime',
        'GRANT SELECT ON ALL TABLES IN SCHEMA public TO rls_reader',
      ].join('; '),
    );
    ...
    const verification = psql(
      restorePg,
      "SELECT CASE WHEN (SELECT count(*) FROM migrations)=1 AND (SELECT count(*) FROM tenant_records)=2 AND EXISTS (SELECT 1 FROM pg_policy WHERE polrelid='tenant_records'::regclass) AND has_table_privilege('genesis_runtime','tenant_records','SELECT') AND NOT has_table_privilege('genesis_runtime','migrations','SELECT') AND NOT has_table_privilege('genesis_runtime','lead_ingest_idempotency','SELECT') AND NOT has_table_privilege('genesis_runtime','lead_command_idempotency','SELECT') AND NOT has_table_privilege('genesis_runtime','lead_follow_up_idempotency','SELECT') AND pg_get_userbyid((SELECT datdba FROM pg_database WHERE datname=current_database()))='genesis_migration' AND pg_get_userbyid((SELECT nspowner FROM pg_namespace WHERE nspname='public'))='genesis_migration' AND NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind IN ('r','p','S') AND pg_get_userbyid(c.relowner)<>'genesis_migration') THEN 'verified' ELSE 'invalid' END",
    ).stdout.trim();
